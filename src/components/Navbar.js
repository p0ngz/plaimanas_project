...
function Navbar(brandBar) {
  ...
  menuBtn.addEventListener("click", () => {
    ...
    // Fit submenu brand viewBox on first open
    if (!submenuBrandFitted && isOpen) {
      const brandText = submenu.querySelector(".navbar-brand-text");
      if (brandText) {
        fitSvgViewBox(brandText);
        submenuBrandFitted = true;
      }
    }
  });
  ...
  nav.querySelectorAll(".navbar-submenu-toggle").forEach((toggle) => {
    toggle.addEventListener("click", (e) => {
      ...
      if (isActive) {
        link.classList.replace("navbar-submenu-link-thick", "navbar-submenu-link");
        sublinks.classList.remove("active");
        toggle.classList.remove("active");
      } else {
        link.classList.replace("navbar-submenu-link", "navbar-submenu-link-thick");
